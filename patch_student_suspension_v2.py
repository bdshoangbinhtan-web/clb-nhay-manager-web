...
restore = Path("app/students/[id]/page.tsx.backup")

if not path.exists():
    raise SystemExit(f"Không tìm thấy {path}")
if not restore.exists():
    raise SystemExit("Không tìm thấy page.tsx.backup để khôi phục an toàn.")

# Restore the original file because the previous patch stopped halfway.
shutil.copy2(restore, path)
fresh_backup = Path("app/students/[id]/page.tsx.before-suspension-v2")
shutil.copy2(path, fresh_backup)
...
if "const [suspendModal" not in text:
  ...
  const [processingAdjustment, setProcessingAdjustment] = useState(false);'''
    ...
    text = text.replace(anchor, states, 1)

if "async function openSuspendModal()" not in text:
    marker = "  async function saveStudent()"
    pos = text.find(marker)
    if pos < 0:
        raise SystemExit("Không tìm thấy saveStudent.")

    helpers = r'''  const adjustmentByTuition = useMemo(() => {
...
if 'student?.status === "active" && status === "inactive"' not in text:
    ...
    saving_pos = text.find("    setSaving(true);", start)
    if saving_pos < 0:
        raise SystemExit("Không tìm thấy setSaving(true) trong saveStudent.")
    guard = '''    if (student?.status === "active" && status === "inactive") {
      await openSuspendModal();
      return;
    }

'''
    text = text[:saving_pos] + guard + text[saving_pos:]
